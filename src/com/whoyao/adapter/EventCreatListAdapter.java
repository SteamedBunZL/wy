 ...
 */
public class EventCreatListAdapter extends BaseAdapter {
	...
	public EventCreatListAdapter(List<EventListItem> list,LayoutInflater inflater,OnClickListener listener) {
		super();
		mList = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(null == convertView){
			...
			if(null != listener){
				holder.tvJoinerMgr.setOnClickListener(listener);
		...
		holder.tvJoinerMgr.setTag(position);
		...
		if(1 == item.getActivitycategory()){
			...
			if(2 == item.getActivityhidestatus() && 3 == item.getActivityprogressstate()){
		...
		if(state ==1 && (prog !=4 || prog == 0) && 0 == item.getJoinnum()){
			holder.vBtnGroup.setVisibility(View.GONE);
		}else{
			holder.vBtnGroup.setVisibility(View.VISIBLE);
		}
		//TODO
		//state 0：未知 1：未达成 2：达成 3：招募中 4：关闭
		//progress 0：未知 1：即将开始 2：进行中 3：已结束 4：准备中
...
//		}
		...
		if(0 == item.getJoinnum()){
			...
			holder.tvJoinerMgr.setVisibility(View.VISIBLE);
			holder.vJoinerL0.setVisibility(View.VISIBLE);
			holder.vJoinerL1.setVisibility(View.VISIBLE);
			holder.vInfill0.setVisibility(View.GONE);
		}
		...
		if(2 == state && 3 == prog){
			...
			holder.tvValu.setVisibility(View.GONE);
