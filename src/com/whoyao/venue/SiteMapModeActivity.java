...
public class SiteMapModeActivity extends BasicActivity implements
		OnClickListener {
	/**
	 * MapView 是地图主控件
	 */
...
//	private EventMapTModel mapModel = new EventMapTModel();
	...
	// 图层
	private SiteOverlay siteOverlay;
	private TextView tvCity;
	private SiteSearchTModel mapModel;
...
//		tvCity.setVisibility(View.GONE);
	...
	private void initMapView() {
		...
		mapModel = (SiteSearchTModel) intent.getSerializableExtra(Extra.SelectedItem);
		mMapView = (MapView) findViewById(R.id.bmapEventView);
		siteOverlay = new SiteOverlay(mMapView);
...
//					mapModel.setLongitude(location.getLongitude());
					...
					Net.cacheRequest(mapModel, WebApi.Venue.PlaceSearch,
							initMapHandler());
		...
		if (initMapHandler == null) {
			initMapHandler = new ResponseHandler(true) {
				...
				public void onSuccess(String content) {
					ArrayList<SiteVenueItem> list = JSON.getObject(content,
							SiteRModel.class).getVenueList();
					int indexOf = mMapView.getOverlays().indexOf(siteOverlay);
					if (-1 != indexOf) {
						mMapView.getOverlays().remove(indexOf);
					}
					siteOverlay.setSiteData(list);
					mMapView.getOverlays().add(siteOverlay);
					mMapView.refresh();
				}
