 ...
 * @param <item>
 */
public class TopicPhotoCancelActivity<item> extends BasicActivity implements OnClickListener {
...
//				EventPhotoAddActivity.titles.remove(position);
				TopicAddActivity.photos.remove(position);
				mAdatper.notifyDataSetChanged();
				if (TopicAddActivity.photos.isEmpty()) {
					finish();
				} else {
					if (position < TopicAddActivity.photos.size()) {
						mViewFlow.setSelection(position);
						tvTitle.setText(position + 1 + " of " + TopicAddActivity.photos.size());
					} else {
						mViewFlow.setSelection(position - 1);
						tvTitle.setText(position + " of " + TopicAddActivity.photos.size());
					}
				}
			}
		});
	...
	class ViewPagerAdatper extends BaseAdapter {
		...
		@Override
		public int getCount() {
			return TopicAddActivity.photos.size();
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			L.i("whoyao",TopicAddActivity.photos.get(position)+"");
			loader.loadBitmap(TopicAddActivity.photos.get(position), handler.iv);
			return convertView;
		}
