 ...
 * @param <item>
 */
public class VenuPhotoCancelActivity<item> extends BasicActivity implements OnClickListener {
...
//				VenueMoeInfoActivity.photos.remove(position);
//				mAdatper.notifyDataSetChanged();
//				if (VenueMoeInfoActivity.photos.isEmpty()) {
//					finish();
//				} else {
//					if (position < VenueMoeInfoActivity.photos.size()) {
//						mViewFlow.setSelection(position);
//						tvTitle.setText(position + 1 + " of " + VenueMoeInfoActivity.photos.size());
//					} else {
//						mViewFlow.setSelection(position - 1);
//						tvTitle.setText(position + " of " + VenueMoeInfoActivity.photos.size());
//					}
//				}
//			}
//		});
//		dialog.setRightButton("取消", null);
	...
	private void initView() {
		...
		findViewById(R.id.page_btn_delete).setVisibility(View.GONE);
	...
	class ViewPagerAdatper extends BaseAdapter {
		...
		@Override
		public int getCount() {
			return VenueMoeInfoActivity.photos.size();
		}
...
//			loader.loadBitmap(VenueMoeInfoActivity.photos.get(position), handler.iv);
			loader.loadBitmapByID(VenueMoeInfoActivity.photos.get(position), handler.iv);
			return convertView;
		}
