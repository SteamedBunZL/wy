...
public class InviteMySendAdapter extends BaseAdapter {
	private List<InviteDetailListItem> mList;
	private LayoutInflater inflater;
	private static String[] states = {"未处理","未处理","已拒绝","已过期","已接受","全部"};
	private static String[] order = {"按时间由远及近","按时间由近及远"};
	private OnClickListener listener;

	public InviteMySendAdapter(List<InviteDetailListItem> list,LayoutInflater inflater,OnClickListener listener) {
		super();
		mList = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.tvinviteSendPerson.setText("受邀人："+item.getReciveUsreName());
